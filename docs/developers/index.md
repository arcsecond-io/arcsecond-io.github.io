...
the cloud or on your own [Arcsecond.local](/local/) install — there are several
open, associated services and projects you can build on.

* [APIs](./apis.md) - the backend RESTful APIs every Arcsecond server exposes.
* [aa-js](./aa-js.md) - our open-source library of astronomical algorithms in Typescript.
* [CLI](/cli/) - our open-source Command-Line Interface (a.k.a. the `arcsecond` terminal command), and the tool to set up and run Arcsecond.local.
