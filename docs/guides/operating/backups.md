---
title: "Backups on a self-hosted install"
visibility: public
audience: operator
tier: guide
source: handwritten
...
`SHARED_DATA_PATH` during `arcsecond setup`:
...
| `docker-compose.yml`              | Only if you have customised it — otherwise `arcsecond setup` rewrites it from the packaged template. |
