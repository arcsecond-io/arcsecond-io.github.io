...
If you are targeting a specific organisation by its subdomain, pass it:

```python
api = ArcsecondAPI(config, subdomain="my-observatory")
```

On a self-hosted [Arcsecond.local](/concepts/arcsecond-local) install, the subdomain is always
`"local"`:
...
Once authenticated, move to [Resources](./python-resources) for the generic CRUD helpers,
