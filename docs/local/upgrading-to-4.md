...
[Troubleshooting](/local/troubleshooting) page used to: a port already taken, a user not in the `docker` group, Docker
Desktop not running.

The one `docker` command left is the login to the image registry at installation time.
...
stays stopped. `arcsecond proxy start --no-autostart` opts out. See [Live-Image Proxy](/local/webcam).
